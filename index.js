...
(async function main() {
    ...
    const vpcId = vpc.id
    const subnets = await aws.ec2.getSubnets({filters: [{name: 'vpc-id', values: [vpcId]}]})
    ...
    const repo = new awsx.ecr.Repository('node-api-repo', {
        forceDelete: true,
        name: 'node-api-repo',
        lifecyclePolicy: {rules: [{maximumNumberOfImages: 10, tagStatus: 'any'}]}
    })
    ...
    image.imageUri.apply(imageUri => {
        ...
        const getIntegration = new aws.apigatewayv2.Integration('ecs-get-integration', {
            apiId: api.id,
